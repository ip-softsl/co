...
/** Call node.updateRwSetting() with Aether 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Aether 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/aether/aether/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because aether 22.x will not try to read these.
...
//! Convert enabled/size values to aether -prune setting.
...
//! Get pruning enabled value to show in GUI from aether -prune setting.
...
//! Get pruning size value to show in GUI from aether -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAetherUnit")) {
        settings.setValue("DisplayAetherUnit", QVariant::fromValue(AetherUnit::ATR));
    }
    QVariant unit = settings.value("DisplayAetherUnit");
    if (unit.canConvert<AetherUnit>()) {
        m_display_aether_unit = unit.value<AetherUnit>();
    } else {
        m_display_aether_unit = AetherUnit::ATR;
        settings.setValue("DisplayAetherUnit", QVariant::fromValue(m_display_aether_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if aether.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_aether_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AetherUnit>() == m_display_aether_unit) return;
    m_display_aether_unit = new_unit.value<AetherUnit>();
    QSettings settings;
    settings.setValue("DisplayAetherUnit", QVariant::fromValue(m_display_aether_unit));
    Q_EMIT displayUnitChanged(m_display_aether_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/aether/aether/pull/8273
    ...
    // (https://github.com/aether-core/gui/issues/567).
