// Copyright (c) 2011-2022 The Aether Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/aethergui.h>

#include <qt/aetherunits.h>
...
const std::string AetherGUI::DEFAULT_UIPLATFORM =
...
AetherGUI::AetherGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AetherGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AetherGUI::showModalOverlay);
...
void AetherGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Aether address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aether: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AetherGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AetherGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AetherGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Aether addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Aether addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Aether Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Aether Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a aether: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Aether command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AetherGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AetherGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AetherGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AetherGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AetherGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AetherGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AetherGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AetherGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &AetherGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &AetherGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AetherGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AetherGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AetherGUI::showDebugWindow);
}

void AetherGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void AetherGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AetherGUI::setCurrentWalletBySelectorIndex);
...
void AetherGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AetherGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AetherGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AetherGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AetherGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AetherGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void AetherGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AetherGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AetherGUI::removeWallet);
...
WalletController* AetherGUI::getWalletController()
{
    return m_wallet_controller;
}

void AetherGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AetherGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AetherGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AetherGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AetherGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AetherGUI::incomingTransaction);
    connect(this, &AetherGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AetherGUI::removeWallet(WalletModel* walletModel)
{
...
void AetherGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AetherGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AetherGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AetherGUI::setWalletActionsEnabled(bool enabled)
{
...
void AetherGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AetherGUI::toggleHidden);
...
void AetherGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AetherGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AetherGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AetherGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AetherGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AetherGUI::openClicked()
{
...
void AetherGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AetherGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AetherGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AetherGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AetherGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AetherGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AetherGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AetherGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Aether network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AetherGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AetherGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AetherGUI::updateHeadersSyncProgressLabel()
{
...
void AetherGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AetherGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AetherGUI::quitRequested);
...
void AetherGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AetherGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AetherGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AetherGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AetherGUI::show);
                e->ignore();
            }
        }
    }
...
void AetherGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AetherGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AetherGUI::incomingTransaction(const QString& date, AetherUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AetherUnits::formatWithUnit(unit, amount, true));
...
void AetherGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AetherGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AetherGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AetherGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AetherGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AetherGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AetherGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AetherGUI::updateProxyIcon()
{
    std::string ip_port;
...
void AetherGUI::updateWindowTitle()
{
...
void AetherGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AetherGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AetherGUI::detectShutdown()
{
...
void AetherGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AetherGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AetherGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AetherGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AetherGUI::subscribeToCoreSignals()
{
...
void AetherGUI::unsubscribeFromCoreSignals()
{
...
bool AetherGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AetherUnit> units = AetherUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AetherUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AetherUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AetherUnit u : AetherUnits::availableUnits()) {
        menu->addAction(AetherUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AetherUnit newUnits)
{
    setText(AetherUnits::longName(newUnits));
}
