...
#include <qt/aetheraddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Aether address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AetherAddressEntryValidator(parent));
    widget->setCheckValidator(new AetherAddressCheckValidator(parent));
}
...
bool parseAetherURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no aether: URI
    if(!uri.isValid() || uri.scheme() != QString("aether"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!AetherUnits::parse(AetherUnit::ATR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseAetherURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAetherURI(uriInstance, out);
}

QString formatAetherURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("aether:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AetherUnits::format(AetherUnit::ATR, info.amount, false, AetherUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAetherConf()
{
    ...
    /* Open aether.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aether.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aether (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Aether (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Aether*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "aether.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("aether-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a aether.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Aether\n";
        else
            optionFile << strprintf("Name=Aether (%s)\n", ChainTypeToString(chain));
