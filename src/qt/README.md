This directory contains the source code for the Aether Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Aether UI.
...
- ClientModel is used by the main application `aethergui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a aether:... link from a browser).
...
* UI elements like AetherAmountField, which inherit from QWidget.
* `aetherstrings.cpp`: automatically generated
* `aetherunits.(h/cpp)`: ATR / mATR / etc. handling
* `callback.h`
...
**Note:** Do not change `local/aether_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `aether-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `aether-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `aether-qt`
9. You're all set! Start developing, building, and debugging the Aether Core GUI
