...
#include <config/aether-config.h>
#endif
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<AetherUnit>("AetherUnit");
#else
    qRegisterMetaType<AetherUnit>("AetherUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. aether_de.qm (shortcut "de" needs to be defined in aether.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. aether_de_DE.qm (shortcut "de_DE" needs to be defined in aether.qrc)
...
static const char* qt_argv = "aether-qt";
...
void AetherApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AetherApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AetherGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void AetherApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool AetherApplication::createOptionsModel(bool resetSettings)
{
...
void AetherApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new AetherGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AetherGUI::quitRequested, this, &AetherApplication::requestShutdown);
...
void AetherApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void AetherApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AetherApplication::baseInitialize()
{
    return node().baseInitialize();
}

void AetherApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AetherApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AetherApplication::handleRunawayException);
    connect(this, &AetherApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AetherApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void AetherApplication::parameterSetup()
{
...
void AetherApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void AetherApplication::requestInitialize()
{
...
void AetherApplication::requestShutdown()
{
...
void AetherApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // aether: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AetherGUI::handlePaymentRequest);
            connect(window, &AetherGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AetherApplication::handleRunawayException(const QString &message)
{
...
void AetherApplication::handleNonFatalException(const QString& message)
{
...
WId AetherApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool AetherApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AetherGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(aether);
    Q_INIT_RESOURCE(aether_locale);
    ...
    /// 6-7. Parse aether.conf, determine network, switch to network specific
    ...
    // aether: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
