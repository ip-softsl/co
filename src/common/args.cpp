...
const char * const BITCOIN_CONF_FILENAME = "aether.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //aether-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Aether
    // macOS: ~/Library/Application Support/Aether
    // Unix-like: ~/.aether
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Aether";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Aether";
#else
    // Unix-like
    return pathRet / ".aether";
#endif
#endif
}
