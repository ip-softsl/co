...
"""Check that it's not possible to start a second aetherd instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(AetherTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second aetherd instance using the same datadir")
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second aetherd instance using the same wallet")
                if descriptors:
