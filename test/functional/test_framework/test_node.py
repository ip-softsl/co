#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Aether Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for aetherd node under test"""
...
class TestNode():
    """A class for representing a aetherd node under test.
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, aetherd, aether_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        """
        Kwargs:
        ...
        self.aetherconf = os.path.join(self.datadir, "aether.conf")
        ...
        # Configuration for logging is set as command-line args rather than in the aether.conf file.
        # This means that starting a aetherd using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(aether_cli, self.datadir)
    ...
    def __del__(self):
        # Ensure that we don't leave any aetherd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time aetherd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by aetherd, and
        ...
        self.running = True
        self.log.debug("aetherd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the aetherd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'aetherd exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; aetherd is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to aetherd after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; aetherd is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into aetherd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to aetherd
        expected_msg: regex that stderr should match when aetherd fails

        Will throw if aetherd starts without an error.
        Will throw if an expected_msg is provided and it does not match aetherd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'aetherd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'aetherd should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to aether-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.aethercli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with aether-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run aether-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running aether-cli {}".format(p_args[2:]))
