...
class ConfArgsTest(AetherTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in aether.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "aether_bad.conf"
        ...
        with open(os.path.join(self.nodes[0].datadir, 'aether.conf'), 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "aether_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by aetherd.
        env, default_datadir = util.get_temp_default_datadir(pathlib.Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a aether.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = pathlib.Path(node.aetherconf).read_text()
        conf_path = default_datadir / "aether.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a aether.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "aether.conf" file which is ignored, because a '
        ...
        # the same aether.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir}/aether.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when aether.conf in the default data directory sets another datadir '
                      'and it contains a different aether.conf file that would be ignored')
        ...
        # aether.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "aether.conf").write_text(f"datadir={node.datadir}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "aether.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/aether.conf" from data directory "{default_datadir}" '
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "aether.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_aether.conf")
