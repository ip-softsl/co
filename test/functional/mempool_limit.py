...
class MempoolLimitTest(AetherTestFramework):
    ...
    def run_test(self):
        ...
        worst_feerate_atrvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_atrvb = min(worst_feerate_atrvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_fee = (worst_feerate_atrvb) * (target_weight_each // 4) - Decimal("0.00001")
        ...
        assert_greater_than(worst_feerate_atrvb, (parent_fee + child_fee) / (tx_parent_just_below["tx"].get_vsize() + tx_child_just_above["tx"].get_vsize()))
