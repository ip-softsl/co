...
def parse_config_into_atr_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Aether Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/aether_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/aether_config.h'), "w", encoding="utf8") as atr_config:
        atr_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Aether-core msbuild configuration initialiser.')
    ...
    parse_config_into_atr_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/aether_config.h'), os.path.join(SOURCE_DIR, 'config/aether-config.h'))
