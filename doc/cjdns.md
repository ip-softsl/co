# CJDNS support in Aether Core

It is possible to run Aether Core over CJDNS, an encrypted IPv6 network that
...
done outside of Aether Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Aether network and individual nodes.
...
before it will work with your Aether Core node. This is described in steps
...
## Run Aether Core with CJDNS

Once you are connected to the CJDNS network, the following Aether Core
...
When enabled, this option tells Aether Core that it is running in an
...
IPv6 local network. This helps Aether Core perform better address management:
...
CJDNS support was added to Aether Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `aether-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/aether/aether/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Aether Core

There are several ways to see your CJDNS address in Aether Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `aether-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `aether-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `aether-cli help getnodeaddresses` for details.
