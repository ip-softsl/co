...
Below are some notes on how to build Aether Core for Windows.

The options known to work for building Aether Core on Windows are:
...
Note that for WSL the Aether Core source path MUST be somewhere in the default mount file system, for
example /usr/src/aether, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\aether`, for example:

    make install DESTDIR=/mnt/c/workspace/aether
