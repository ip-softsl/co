Aether Core version 0.11.0 is now available from:

  <https://aether.org/bin/aether-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer ATR/kB fee to be
...
For example, add the following to `aether.conf`:
...
related to the blockchain in the aether system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Aether Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Aether Core will 
...
For an example script see [aether-submittx](https://github.com/laanwj/aether-submittx).
...
- #5334 `cf87536` libaetherconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new aether-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AetherAmountField
- #5753 `0f44672` Add aether logo to about screen
...
- #5809 `46bfbe7` Add aether-cli man page
...
- #5849 `41259ca` contrib/init/aetherd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Aether-Qt.app to Aether-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for aether-tx)
