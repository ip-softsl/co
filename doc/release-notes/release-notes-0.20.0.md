...
Aether Core version 0.20.0 is now available from:

  <https://aethercore.org/bin/aether-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Aether Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Aether Core version 0.18.0.
...
- Testing or debugging of implementations of the Aether P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Aether Core. Aether Core logs debug messages
...
- OpenSSL is no longer used by Aether Core.  (#17265)

- BIP70 support has been fully removed from Aether Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/aether/aether/issues/16599),
  [PR #16702](https://github.com/aether/aether/pull/16702), and the
  `aetherd help` for more information.  This option is experimental and
...
- All custom settings configured when Aether Core starts are now
...
- The "Start Aether Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://aethercore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Aether Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Aether Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/aether-core/HWI) for
  signing.  Future versions of Aether Core should support a GUI option
...
- Aether Core's automatically-generated source code documentation is
  now available at https://doxygen.aethercore.org.  (#17596)
...
- The `aether-cli` utility used with the `-getinfo` parameter now
...
- The `aether-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `aetherstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_aether-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_aether` binary (hebasto)
...
- #18702 Fix ASLR for aether-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate aether schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest aether.conf (sdaftuar)
...
- #18022 Fix appveyor `test_aether` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (aether-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone aether-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Aether Core files (hebasto)
...
- #17119 Fix broken aether-cli examples (andrewtoth)
...
- #17503 Remove bitness from aether-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.aethercore.org (laanwj)
...
- #17648 Rename wallet-tool references to aether-wallet (hel-o)
...
- #18341 Replace remaining literal ATR with `CURRENCY_UNIT` (domob1812)
...
- #16115 On aetherd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from aether-cli -getinfo (malevolent)
