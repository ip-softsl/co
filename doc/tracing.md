# User-space, Statically Defined Tracing (USDT) for Aether Core

Aether Core includes statically defined tracepoints to allow for more
...
**Note**: Aether Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Aether Core
...
Multiple tools can list the available tracepoints in a `aetherd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Aether Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/aetherd
stap net        outbound_message 0x0000000000107c05 /src/aetherd
stap validation block_connected  0x00000000002fb10c /src/aetherd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Aether Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/aetherd | grep NT_STAPSDT -A 4 -B 2
