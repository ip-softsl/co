# PSBT Howto for Aether Core

Since Aether Core 0.17, an RPC interface exists for Partially Signed Aether
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/aether/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Aether transactions that are not fully signed
...
#### Multisig with multiple Aether Core instances
...
Aether Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `aether-cli` is used.
...
Later, when *V* ATR has been received on *Amulti*, and Bob and Carol want to
