# Bootstrappable Aether Core Builds

This directory contains the files necessary to perform bootstrappable Aether
Core builds.
...
To build Aether Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Aether Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/aether-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/aether-core/guix.sigs).
