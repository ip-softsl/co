#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Aether Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing aether-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='aether-cli', help='aether-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Aether address to which the faucet should send')
...
parser.add_argument('aether_cli_args', nargs='*', help='Arguments to pass on to aether-cli (default: -signet)')

args = parser.parse_args()

if args.aether_cli_args == []:
    args.aether_cli_args = ['-signet']


def aether_cli(rpc_command_and_params):
    argv = [args.cmd] + args.aether_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = aether_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = aether_cli(['getnewaddress', 'faucet', 'bech32'])
