...
"""Script for verifying Aether Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from aethercore.org and aether.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/aether-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://aethercore.org"
HOST2 = "https://aether.org"
VERSIONPREFIX = "aether-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Aether community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"aether_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by aethercore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the aether release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes aether.org lags behind aethercore.org.)')
    )
