...
Tracing (USDT) support for Aether Core on Linux using. For more information on
USDT support in Aether Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `aetherd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `aetherd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Aether Core USDT documentation] on how to list available tracepoints in your
`aetherd` binary.

[Aether Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/aetherd
```
...
Aether P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/aetherd
```
...
In a different terminal, starting Aether Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/aetherd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/aetherd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/aetherd
```
