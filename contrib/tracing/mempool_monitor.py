...
""" Example logging Aether Core mempool events using the mempool:added,
...
def main(aetherd_path):
    aetherd_with_usdts = USDT(path=str(aetherd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    aetherd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    aetherd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    aetherd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    aetherd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[aetherd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/aetherd")
        exit(1)
